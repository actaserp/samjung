		...
		try {
			...
			if ("3".equals(docuTy)) {
				...
				for (Map.Entry<String, List<Map<String, Object>>> entry : groupMap.entrySet()) {
					...
					log.info("[DZ전송] ── 그룹[{}] 채번된 menuSq={}", entry.getKey(), menuSq);
					...
					log.info("[DZ전송] ── 더존 API 호출 | 그룹={} menuSq={} 라인수={}",
						entry.getKey(), menuSq, groupLines.size());
					log.info("[DZ전송]    페이로드 상세: {}", payload);

					Map<String, Object> dzRes = douzoneClient.callAutoDocuSave(payload);

					log.info("[DZ전송] ── 더존 API 응답: {}", dzRes);
